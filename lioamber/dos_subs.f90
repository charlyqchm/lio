...
subroutine init_PDOS (M)
    use DOS_data, only: pdos_calc, pdos_allb , pdos, pdos_nuc, pdos_base,      &
                        pdos_natoms, pdos_nbases, pdos_b
...
subroutine build_PDOS(coef_mat, overlap, M, M_total, Nuc)
   ...
   use dftb_data, only: dftb_calc, MTB, MDFTB
...
subroutine write_DOS (M, NCO, morb_energy)
   use DOS_data, only: dos_calc, pdos_calc, pdos, dos_nsteps , dos_Eref,        &
                       dos_sigma, pdos_b, pdos_allb, pdos_nbases

   implicit none
   integer, intent(in) :: M, NCO
   real*8, intent(in)  :: morb_energy(M)
   ...
   pexpf = 1.0d0 !1.0d0/(dos_sigma*dsqrt(2.0d0*pi)*M)
   expf  = -1.0d0/(2.0d0*dos_sigma**2)

   min_e = morb_energy(1) - (morb_energy(M)-morb_energy(1))/M
   max_e = morb_energy(M) - (morb_energy(M)-morb_energy(1))/M
